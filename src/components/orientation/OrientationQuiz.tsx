...
export default function OrientationQuiz({ sectionNumber, questions, passingScore, onPass, onFail }: OrientationQuizProps) {
  ...
  const handleSubmit = () => {
    ...
    questions.forEach((q) => {
      if (answers[q.id] === q.correct_answer) correct += q.points;
    });
    const pct = totalPoints > 0 ? Math.round((correct / totalPoints) * 100) : 0;
    setScore(pct);
    setSubmitted(true);
    if (pct >= passingScore) {
      onPass(pct);
              ...
              {q.options.map((opt) => {
                ...
                return (
                  <Button
                    ...
                    disabled={submitted}
                  >
                    {submitted && isCorrect && <CheckCircle2 className="w-4 h-4 mr-2 text-success shrink-0" />}
                    {submitted && isSelected && !isCorrect && <XCircle className="w-4 h-4 mr-2 text-destructive shrink-0" />}
